...
namespace Utilities.CodeGeneration
{
    ...
    public sealed class OptionSetCustomisationService : ICustomizeCodeDomService
    {
        /// <summary>
        ...
        public void CustomizeCodeDom(CodeCompileUnit codeUnit, IServiceProvider services)
        {
            ...
            foreach (CodeNamespace codeNamespace in codeUnit.Namespaces)
            {
                foreach (CodeTypeDeclaration codeTypeDeclaration in codeNamespace.Types)
                {
                    ...
                    {
                        //Clear Attributes and base tyoes
                                    ...
                                    if (attributeMetadata != null)
                                    {
                                        ...
                                        switch (attributeMetadata.AttributeType.Value)
                                        {
                                            case AttributeTypeCode.Boolean:
                                                ...
                                                MarkAsStaticClassWithExtensionMethods(booleanClass);
                                                booleanClass.CustomAttributes.Clear();
                                                booleanClass.BaseTypes.Clear();
        ...
        {
            var sanatisedString = value.Replace("<>", "NotEquals").Replace("!=", "NotEquals").Replace("=", "Equals").Replace("<", "LessThan").Replace(">", "GreaterThan").Replace("+", "Plus");
            sanatisedString = sanatisedString.Replace("\u26D4", "Stop");
